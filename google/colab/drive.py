...
def _env():
    ...
    root_dir = _os.path.realpath(_os.path.join(_os.environ["CLOUDSDK_CONFIG"], "../.."))
...
def _mount(
    mountpoint, force_remount=False, timeout_ms=120000, ephemeral=False, readonly=False, additional_preferences=""
):
    ...
    if _os.environ.get("VERTEX_PRODUCT") == "COLAB_ENTERPRISE":
        raise NotImplementedError("google.colab.drive.mount is not supported in Colab Enterprise.")
    metadata_server_addr = _os.environ["TBE_EPHEM_CREDS_ADDR"] if ephemeral else _os.environ["TBE_CREDS_ADDR"]
    if ephemeral:
    ...
    d.sendline(f"umount -f {mountpoint} || umount {mountpoint}; pkill -9 -x drive")
    ...
    success_watcher = (
        '( while `sleep 0.5`; do if [[ -d "{m}" && "$(ls -A {m})" != "" ]]; ' 'then echo "{s}"; break; fi; done ) &'
    ...
    d.sendline(
        ...
        f"mount_point_path:{mountpoint}" + "," + additional_preferences + " 2>&1 |"
    ...
    while True:
        ...
        if case == 0:
            ...
            d.kill(_signal.SIGKILL)
            extra_reason = ""
            if 0 == _subprocess.call(f'grep -q "{timeout_pattern}" "{dfs_log}"', shell=True):
                extra_reason = (
            ...
            # Now kill bash.
            d.kill(_signal.SIGKILL)
            raise ValueError(str(domain_disabled_drivefs) + ": https://support.google.com/a/answer/7496409")
    ...
    filter_cmd = (
        """nohup bash -c 'tail -n +0 -F "{}" | """ """python3 {} > "{}" ' < /dev/null > /dev/null 2>&1 &"""
    ).format(dfs_log, filter_script, filtered_logfile)
    d.sendline(filter_cmd)
    d.expect(prompt)
    if "ENABLE_DIRECTORYPREFETCHER" in _os.environ:
        d.sendline(
            """nohup bash -c '{d}/directoryprefetcher_binary -mountpoint={mnt}' """ """>> {log} 2>&1 &""".format(
                d=drive_dir,
                mnt=mountpoint,
